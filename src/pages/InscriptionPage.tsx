...
import type { Session, Formation } from '@/types/database';
import {
  Building2,
  User,
  GraduationCap,
...
export function InscriptionPage() {
  ...
  useEffect(() => {
    ...
    dataService.getSessionByToken(token).then((s) => {
      if (!s) {
        setExpired(true);
  ...
  async function handleSubmit() {
    ...
    const result = await dataService.submitInscription(token, {
      ...
      <div className="min-h-screen flex items-center justify-center bg-nikita-gray px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600">
            Ce lien d'inscription n'est pas valide ou a expiré. Contactez l'organisme de formation.
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-nikita-gray px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 mb-2">
            ...
            <strong>{formation?.intitule || session?.formation?.intitule || ''}</strong>.
          </p>
          <p className="text-sm text-gray-500">
            Un email de confirmation vous a été envoyé. Notre équipe vous contactera dans les 24h
    ...
    <div className="min-h-screen bg-nikita-gray py-8 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-gray-800">Inscription à une formation</h1>
          <p className="text-sm text-gray-500 mt-1">NIKITA — Organisme certifié Qualiopi</p>
          {formation && (
            <div className="mt-3 inline-block bg-white rounded-lg px-4 py-2 text-sm">
              <span className="text-gray-500">Formation :</span>{' '}
              <span className="font-semibold text-gray-800">{formation.intitule}</span>
              <span className="text-gray-400 mx-2">•</span>
              <span className="text-gray-600">{formation.duree_heures}h</span>
              <span className="text-gray-400 mx-2">•</span>
              <span className="text-gray-600">{formation.modalite}</span>
            </div>
          )}
        </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Input
                    ...
                    onChange={(e) => updateField('nb_salaries', e.target.value)}
                    placeholder="Ex: 25"
                  />
                  <Input
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Input
                    ...
                    error={errors.representant_prenom}
                    placeholder="Jean"
                  />
                  <Input
                    label="Nom *"
                ...
                <Input
                  ...
                  onChange={(e) => updateField('representant_fonction', e.target.value)}
                  error={errors.representant_fonction}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Input
                    ...
                    onChange={(e) => updateField('nb_participants', e.target.value)}
                    error={errors.nb_participants}
                    placeholder="5"
                  />
                  <SelectField
                    label="Nombre"
                    value={form.mode_participants}
                    onChange={(e) => updateField('mode_participants', e.target.value)}
                    options={[
                      { value: 'exact', label: 'Exact' },
                      { value: 'estimation', label: 'Estimation' },
                      { value: 'ulterieur', label: 'À préciser' },
                    ]}
                  />
                </div>
                <Input
                  ...
                  type="date"
                  value={form.date_souhaitee_debut}
                  onChange={(e) => updateField('date_souhaitee_debut', e.target.value)}
                />
                <SelectField
                  label="Financement OPCO"
                  value={form.opco_financement}
                  onChange={(e) => updateField('opco_financement', e.target.value)}
                  options={[
                    { value: 'oui', label: 'Oui' },
                    { value: 'non', label: 'Non' },
                ...
                {form.opco_financement === 'oui' && (
                  <Input
                    label="Nom de l'OPCO *"
                    value={form.opco_nom}
                    onChange={(e) => updateField('opco_nom', e.target.value)}
                    error={errors.opco_nom}
                    placeholder="Ex: Afdas, Atlas, Opco Commerce..."
                  />
                )}
                <SelectField
                  ...
                  options={[
                    { value: 'non', label: 'Non' },
                    { value: 'oui', label: 'Oui, adaptation nécessaire' },
                  ]}
                />
                <Textarea
                  label="Besoins spécifiques / contexte"
                  value={form.recueil_besoins}
                  onChange={(e) => updateField('recueil_besoins', e.target.value)}
                  placeholder="Décrivez vos objectifs, votre contexte métier, vos attentes spécifiques..."
                  rows={4}
                />
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      <p className="font-medium">{form.raison_sociale}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">SIRET</span>
                      <p className="font-medium font-mono text-xs">{form.siret}</p>
                    </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      <p className="font-medium">
                        {form.representant_prenom} {form.representant_nom}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-500">Fonction</span>
                      <p className="font-medium">{form.representant_fonction}</p>
                    </div>
                    <div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      <p className="font-medium">{form.nb_participants}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Date souhaitée</span>
                      <p className="font-medium">{form.date_souhaitee_debut || 'Non précisée'}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-medium">{form.handicap === 'oui' ? 'Oui' : 'Non'}</p>
                    </div>
                  </div>
                  {form.recueil_besoins && (
                    <div className="text-sm">
                      <span className="text-gray-500">Besoins</span>
                      <p className="font-medium">{form.recueil_besoins}</p>
                    </div>
                  )}
                </div>
                ...
                <p className="text-[10px] text-gray-400 text-center">
                  En confirmant, vous acceptez que vos données soient traitées conformément à notre
