...
export function ClientsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return mockClients.filter((c) => c.raison_sociale.toLowerCase().includes(q) || c.siret.includes(q) || c.email.toLowerCase().includes(q) || c.ville.toLowerCase().includes(q));
  }, [search]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-500">{mockClients.length} client{mockClients.length > 1 ? 's' : ''}</span>
      </div>
      ...
      <Card>
        {filtered.length === 0 ? (
          <EmptyState icon={<Building2 size={48} />} title="Aucun client" description="Aucun client ne correspond à votre recherche." />
        ) : (
