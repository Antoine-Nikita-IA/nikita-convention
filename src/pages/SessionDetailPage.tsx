...
export function SessionDetailPage() {
  ...
  const [currentStatus, setCurrentStatus] = useState<SessionStatus>('en_attente');
  ...
  const [validateForm, setValidateForm] = useState({ dates_formation: '', lieu: '', ville: '', formateurs: '', montant_ht: '' });
  const [stagiaireForm, setStagiaireForm] = useState({ prenom: '', nom: '', email: '', fonction: '' });
  const [sessionLogs, setSessionLogs] = useState<{ id: string; ancien_statut: string; nouveau_statut: string; note: string | null; created_at: string }[]>([]);
  ...
  async function loadData() {
    ...
    setLoading(true);
    try {
      const [sess, org] = await Promise.all([
        dataService.getSessionById(id),
        dataService.getOrganisme(),
      ]);
      if (!sess) { setLoading(false); return; }
      setSession(sess);
      setOrganisme(org);
      setCurrentStatus(sess.status);
      setValidateForm({
        dates_formation: sess.dates_formation || '',
        lieu: sess.lieu || '',
        ville: sess.ville || '',
        formateurs: sess.formateurs || '',
  ...
  async function refreshEmailsAndStagiaires() {
    if (!id) return;
    const [emails, stag] = await Promise.all([
      dataService.getEmailLogsBySessionId(id),
      dataService.getStagiairesBySessionId(id),
    ]);
    setEmailLogs(emails);
  ...
  if (!session) {
    return (
      <div className="text-center py-12">
        ...
        <Button variant="outline" onClick={() => navigate('/sessions')} className="mt-4">Retour aux sessions</Button>
      </div>
    );
  }
  ...
  async function handleSendInscriptionLink() {
    ...
    try {
      ...
      if (result.success) {
        ...
        await refreshEmailsAndStagiaires();
  ...
  async function handleValidate() {
    ...
    try {
      ...
      if (result) {
        ...
        setSession({ ...session!, status: 'valide', ...validateForm, montant_ht: validateForm.montant_ht ? parseFloat(validateForm.montant_ht) : session!.montant_ht });
  ...
  async function handleGenerateConvention() {
    ...
    try {
      ...
      if (result.success && result.convention) {
        ...
        setSession({ ...session!, status: 'convention_generee', convention_numero: result.convention.numero });
  ...
  async function handleSendConvention() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSession({ ...session!, status: 'envoye' });
        await refreshEmailsAndStagiaires();
  ...
  async function handleCancel() {
    ...
    try {
      ...
      setSession({ ...session!, status: 'annule' });
  ...
  async function handleAddStagiaire() {
    ...
    setActionLoading(true);
    try {
      await dataService.addStagiaire({ session_id: id!, client_id: client?.id || '', ...stagiaireForm });
      await refreshEmailsAndStagiaires();
  ...
  function copyLink(type: 'inscription' | 'convention' | 'suivi') {
    ...
    const urls = { inscription: dataService.getInscriptionUrl(token), convention: dataService.getConventionUrl(token), suivi: dataService.getSuiviUrl(token) };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/sessions')} className="p-2 rounded-lg hover:bg-gray-100 transition-colors"><ArrowLeft size={20} /></button>
        <div className="flex-1">
          <h1 className="text-xl font-bold text-gray-800">{session.formation?.intitule || 'Session'}</h1>
          <p className="text-sm text-gray-500">{session.convention_numero || `Session #${session.id.slice(0, 8)}`}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Client */}
          <Card>
            <CardHeader><h2 className="text-sm font-semibold text-gray-700">Client</h2></CardHeader>
            <CardContent>
              {client ? (
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div><span className="text-gray-500">Raison sociale</span><p className="font-medium">{client.raison_sociale}</p></div>
                  <div><span className="text-gray-500">SIRET</span><p className="font-medium font-mono text-xs">{client.siret}</p></div>
                  <div><span className="text-gray-500">Email</span><p className="font-medium">{client.email}</p></div>
                  <div><span className="text-gray-500">Téléphone</span><p className="font-medium">{client.telephone}</p></div>
                  <div><span className="text-gray-500">Représentant</span><p className="font-medium">{client.representant_prenom} {client.representant_nom}</p></div>
                  <div><span className="text-gray-500">Participants</span><p className="font-medium">{client.nb_participants}</p></div>
                  <div className="col-span-2"><span className="text-gray-500">Adresse</span><p className="font-medium">{client.adresse}, {client.code_postal} {client.ville}</p></div>
                  {client.opco_financement && <div className="col-span-2"><span className="text-gray-500">OPCO</span><p className="font-medium text-blue-600">{client.opco_nom || 'Oui'}</p></div>}
                  {client.recueil_besoins && <div className="col-span-2"><span className="text-gray-500">Besoins exprimés</span><p className="font-medium text-sm">{client.recueil_besoins}</p></div>}
                </div>
              ) : (
          ...
          {/* Formation */}
          <Card>
            <CardHeader><h2 className="text-sm font-semibold text-gray-700">Formation</h2></CardHeader>
            <CardContent>
              {session.formation ? (
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div><span className="text-gray-500">Intitulé</span><p className="font-medium">{session.formation.intitule}</p></div>
                  <div><span className="text-gray-500">Durée</span><p className="font-medium">{session.formation.duree_heures}h</p></div>
                  <div><span className="text-gray-500">Modalité</span><p className="font-medium">{session.formation.modalite}</p></div>
                  <div><span className="text-gray-500">Tarif HT / pers.</span><p className="font-medium">{formatMoney(session.formation.tarif_ht)}</p></div>
                  <div><span className="text-gray-500">Dates</span><p className="font-medium">{session.dates_formation || '—'}</p></div>
                  <div><span className="text-gray-500">Lieu</span><p className="font-medium">{session.lieu || '—'}{session.ville ? `, ${session.ville}` : ''}</p></div>
                  {session.formateurs && <div className="col-span-2"><span className="text-gray-500">Formateurs</span><p className="font-medium">{session.formateurs}</p></div>}
                </div>
          ...
          {session.notes_internes && (
            <Card>
              <CardHeader><h2 className="text-sm font-semibold text-gray-700">Notes internes</h2></CardHeader>
              <CardContent><p className="text-sm text-gray-600">{session.notes_internes}</p></CardContent>
            </Card>
          )}
        </div>
        ...
        <div className="space-y-6">
          ...
          {/* Email history */}
          <Card>
            <CardHeader><h2 className="text-sm font-semibold text-gray-700">Historique emails</h2></CardHeader>
            <CardContent>
              {emailLogs.length > 0 ? (
                <div className="space-y-3">
                  {emailLogs.map((log) => (
                    <div key={log.id} className="flex items-start gap-3 text-sm">
                      {log.statut === 'envoye' ? <CheckCircle size={16} className="text-green-500 mt-0.5 shrink-0" /> : log.statut === 'erreur' ? <XCircle size={16} className="text-red-500 mt-0.5 shrink-0" /> : <AlertCircle size={16} className="text-yellow-500 mt-0.5 shrink-0" />}
                      <div className="min-w-0">
                        <p className="font-medium truncate">{log.sujet}</p>
                        <p className="text-xs text-gray-500">{log.destinataire} — {formatDate(log.created_at)}</p>
      ...
      <Modal open={showSendLink} onClose={() => setShowSendLink(false)} title="Envoyer le lien d'inscription"
        footer={<><Button variant="ghost" onClick={() => setShowSendLink(false)}>Annuler</Button><Button onClick={handleSendInscriptionLink} loading={actionLoading} disabled={!linkEmail.trim()} icon={<Send size={16} />}>Envoyer</Button></>}>
        <div className="space-y-4">
          <p className="text-sm text-gray-600">Le client recevra un email avec le lien vers le formulaire d'inscription pour la formation <strong>{session.formation?.intitule}</strong>.</p>
          <Input label="Email du destinataire" type="email" value={linkEmail} onChange={(e) => setLinkEmail(e.target.value)} placeholder="contact@client.fr" />
          <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-500">
            <p className="font-medium mb-1">Lien qui sera envoyé :</p>
            <code className="block bg-white rounded px-2 py-1 text-nikita-pink break-all">{dataService.getInscriptionUrl(session.token)}</code>
          </div>
        </div>
      </Modal>

      {/* Validate session */}
      <Modal open={showValidate} onClose={() => setShowValidate(false)} title="Valider la demande" size="lg"
        footer={<><Button variant="ghost" onClick={() => setShowValidate(false)}>Annuler</Button><Button onClick={handleValidate} loading={actionLoading} icon={<CheckCircle size={16} />}>Valider</Button></>}>
        <div className="space-y-4">
          <p className="text-sm text-gray-600">Complétez ou ajustez les informations de la session avant validation.</p>
          <Input label="Dates de formation" value={validateForm.dates_formation} onChange={(e) => setValidateForm((prev) => ({ ...prev, dates_formation: e.target.value }))} placeholder="Ex: 15-16 avril 2026" />
          <div className="grid grid-cols-2 gap-4">
            <Input label="Lieu" value={validateForm.lieu} onChange={(e) => setValidateForm((prev) => ({ ...prev, lieu: e.target.value }))} placeholder="Centre de formation Lyon" />
            <Input label="Ville" value={validateForm.ville} onChange={(e) => setValidateForm((prev) => ({ ...prev, ville: e.target.value }))} placeholder="Lyon" />
          </div>
          <Input label="Formateurs" value={validateForm.formateurs} onChange={(e) => setValidateForm((prev) => ({ ...prev, formateurs: e.target.value }))} placeholder="Nom(s) des formateurs" />
          <Input label="Montant HT total (€)" type="number" value={validateForm.montant_ht} onChange={(e) => setValidateForm((prev) => ({ ...prev, montant_ht: e.target.value }))} placeholder="Calculé auto si vide" />
          {client && (
            <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-500">
              <p className="font-medium mb-1">Récap client</p>
              <p>{client.raison_sociale} — {client.nb_participants} participants — {client.opco_financement ? `OPCO: ${client.opco_nom}` : 'Sans OPCO'}</p>
              {client.recueil_besoins && <p className="mt-1 text-gray-600">Besoins : {client.recueil_besoins}</p>}
            </div>
          )}
        </div>
      </Modal>
      ...
      {/* Add stagiaire */}
      <Modal open={showAddStagiaire} onClose={() => setShowAddStagiaire(false)} title="Ajouter un stagiaire"
        footer={<><Button variant="ghost" onClick={() => setShowAddStagiaire(false)}>Annuler</Button><Button onClick={handleAddStagiaire} loading={actionLoading} disabled={!stagiaireForm.prenom || !stagiaireForm.nom || !stagiaireForm.email} icon={<UserPlus size={16} />}>Ajouter</Button></>}>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Input label="Prénom *" value={stagiaireForm.prenom} onChange={(e) => setStagiaireForm((prev) => ({ ...prev, prenom: e.target.value }))} placeholder="Prénom" />
            <Input label="Nom *" value={stagiaireForm.nom} onChange={(e) => setStagiaireForm((prev) => ({ ...prev, nom: e.target.value }))} placeholder="Nom" />
          </div>
          <Input label="Email *" type="email" value={stagiaireForm.email} onChange={(e) => setStagiaireForm((prev) => ({ ...prev, email: e.target.value }))} placeholder="email@entreprise.fr" />
          <Input label="Fonction" value={stagiaireForm.fonction} onChange={(e) => setStagiaireForm((prev) => ({ ...prev, fonction: e.target.value }))} placeholder="Ex: Chef de projet" />
        </div>
      </Modal>

      {/* Session logs */}
      <Modal open={showSessionLogs} onClose={() => setShowSessionLogs(false)} title="Historique des changements" size="lg">
        {sessionLogs.length > 0 ? (
          <div className="space-y-3">
            {sessionLogs.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((log) => (
              <div key={log.id} className="flex items-start gap-3 text-sm py-2 border-b border-gray-50 last:border-0">
                ...
                <div>
                  <p className="font-medium">{STATUS_LABELS[log.ancien_statut as SessionStatus] || log.ancien_statut} → {STATUS_LABELS[log.nouveau_statut as SessionStatus] || log.nouveau_statut}</p>
      ...
      {/* Email preview */}
      <Modal open={!!showEmailPreview} onClose={() => setShowEmailPreview(null)}
        title={showEmailPreview === 'convention_envoi' ? 'Aperçu email — Envoi convention' : showEmailPreview === 'convention_signee' ? 'Aperçu email — Convention signée' : showEmailPreview === 'opco_depot' ? 'Aperçu email — Dépôt OPCO' : 'Aperçu email'}
        size="lg" footer={<Button variant="ghost" onClick={() => setShowEmailPreview(null)}>Fermer</Button>}>
        {showEmailPreview && client && organisme && <EmailPreview type={showEmailPreview} session={session} client={client} organisme={organisme} />}
        {!client && <p className="text-sm text-gray-400 text-center py-8">Client requis pour générer l'aperçu email.</p>}
      </Modal>
    </div>
  );
}
